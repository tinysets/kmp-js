...
function indexOf(source: string, sourceCount: number, target: string, targetCount: number) {
    let first = target[0];
...
export function indexOf_java(src: string, test: string) {
    if (!src || !test) {
        console.log(-1);
        return -1;
    }
...
function _indexOf_java(src: string, src_len: number, test: string, test_len: number) {
...
export function str_search(src: string, test: string) {// 返回匹配的首字母索引,没有则返回-1
    if (!src || !test) {
        console.log(-1);
        return -1;
    }
...
function _str_search(src: string, src_len: number, test: string, test_len: number) {
